...
class F2MassFitter:
    ...
    def __build_total_pdf(self):
        """
        Helper function to compoself._background_pdf_se the total pdf
        """
    ...
    def get_std_residuals(self, idx2=0):
        """
    ...
    def plot_std_residuals(self, **kwargs):
        ...
        plt.xlim(limits[0], limits[1])
        plt.xlabel(axis_title)
        ylabel = fr"$\dfrac{{ \mathrm{{data}} - \mathrm{{total \ fit}} }}{{ \sigma_{{ \mathrm{{data}} }} }}$ / {(limits[1]-limits[0])/bins*1000:0.1f} MeV/$c^2$"
        plt.ylabel(ylabel)
        plt.legend(loc='best')
